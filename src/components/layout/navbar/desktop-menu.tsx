				...
				{menu.map((item) =>
					item.submenu ? (
						<NavigationMenuItem key={item.label}>
							<NavigationMenuTrigger className="hover:bg-soft-linen font-semibold text-lg">
								{item.label}
							</NavigationMenuTrigger>
							<NavigationMenuContent>
						...
						<NavigationMenuItem key={item.label}>
							<NavigationMenuLink
								href={item.href || "#"}
								className={cn(
									"group inline-flex h-10 w-max items-center justify-center rounded-md px-4 py-2 text-lg font-semibold transition-colors hover:bg-soft-linen focus:bg-soft-linen focus:outline-none disabled:pointer-events-none disabled:opacity-50",
								)}
							>
								{item.label}
							</NavigationMenuLink>
